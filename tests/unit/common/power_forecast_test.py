...
class PowerForecastTest(TestCase):
    def test__from_json__happy_path(self):
        ...
        self.assertEqual(power_forecast.message_id, uuid.UUID('2bdec96b-be3b-4ba9-afa0-c4a0632cced9'))
        self.assertEqual(power_forecast.start_time, datetime(2023, 8, 2, 12, 48, 42, tzinfo=offset(timedelta(hours=1))))
    ...
    def test__to_json__happy_path(self):
        ...
        power_forecast_element = PowerForecastElement(power_values=[PowerForecastValue(commodity_quantity=CommodityQuantity.NATURAL_GAS_FLOW_RATE,
                                                                                       value_expected=500.2)],
                                                      duration=Duration.from_timedelta(timedelta(seconds=4)))
        power_forecast = PowerForecast(elements=[power_forecast_element],
