...
import pprint
import random
from typing import get_type_hints, Type, get_origin, get_args, Union, TypeVar, Callable, Sequence
import uuid
...
def generate_json_test_data_for_field(field_type: Type):
    if field_type is Duration:
        ...
        value = {'start_of_range': start * 39999, 'end_of_range': (start + offset) * 39999}
    elif field_type is PowerRange:
        start = random.random()
        offset = random.random()
        value = {'start_of_range': start * 39999,
        ...
        value = datetime.datetime(year=random.randint(2020, 2023),
                                  month=random.randint(1, 12),
                                  day=random.randint(1, 28),
                                  hour=random.randint(0, 23),
                                  minute=random.randint(0, 59),
                                  second=random.randint(0, 59),
                                  tzinfo=datetime.timezone(offset=datetime.timedelta(hours=random.randint(0,2))))
...
def dump_test_data_as_constructor_field_for(test_data, field_type: Type) -> str:
    if field_type is Duration:
        ...
        dumped_items = [dump_test_data_as_constructor_field_for(test_data_item, get_list_arg(field_type)) for test_data_item in test_data]
        value = f'[{",".join(dumped_items)}]'
    elif is_optional(field_type):
        value = dump_test_data_as_constructor_field_for(test_data, get_optional_arg(field_type))
    elif is_enum(field_type):
        field_type: Enum
        ...
        raise RuntimeError(f'Please implement dump test data for field type {field_type}')
    return value
...
def dump_test_data_as_constructor_for(test_data: dict, class_: Type) -> str:
    ...
    first = True
    for field_name, field_type in get_type_hints(class_).items():
        value = dump_test_data_as_constructor_field_for(test_data[field_name], field_type)
...
def dump_test_data_as_json_field_for(test_data, field_type: Type):
    if field_type is Duration:
        ...
        value = [dump_test_data_as_json_field_for(item, get_list_arg(field_type)) for item in test_data]
    elif is_optional(field_type):
        value = dump_test_data_as_json_field_for(test_data, get_optional_arg(field_type))
    elif is_enum(field_type):
        field_type: Enum
        value = test_data
    elif field_type is str:
        ...
        raise RuntimeError(f'Please implement dump test data to json for field type {field_type}')
...
def dump_test_data_as_json_for(test_data:dict, class_: Type) -> dict:
    ...
    for field_name, field_type in get_type_hints(class_).items():
        result[field_name] = dump_test_data_as_json_field_for(test_data[field_name], field_type)
...
for class_name, class_ in inspect.getmembers(frbc):
    if inspect.isclass(class_) and issubclass(class_, pydantic.BaseModel):
        ...
        assert_lines = []
        for field_name, field_type in get_type_hints(class_).items():
            assert_test_data = dump_test_data_as_constructor_field_for(test_data[field_name], field_type)

            assert_lines.append(f'self.assertEqual({snake_case(class_name)}.{field_name}, {assert_test_data})')
...
"""
        ...
        with open(f'tests/unit/frbc/{snake_case(class_name)}_test.py', 'w+') as unit_test_file:
