...
class S2FlaskWSServer:
    ...
    def start(self) -> None:
        """
        Start the WebSocket server using Flask's built-in development server.

        Note: This server is synchronous (WSGI) and is not suitable for production.
        It runs each WebSocket connection in a separate thread. For true async
        support and production use, you must run this application with an
        ASGI server like Hypercorn. Example:
        `hypercorn -b 127.0.0.1:8080 examples.example_flask_server:server_instance.app`
        """
        logger.info(
            "Starting S2 Flask WebSocket server with Flask's development WSGI server at: ws://%s:%s%s",
            self.host,
            self.port,
            self.ws_path,
        )
    ...
    def stop(self) -> None:
        ...
        logger.warning("S2FlaskWSServer.stop() is a no-op. The server should be managed by a process manager.")
